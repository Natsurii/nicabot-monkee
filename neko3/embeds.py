...
from neko3.singleton import Singleton
...
class Embed:
    ...
    def url(self, value) -> None:
        if value is not unspecified_field:
            value = self._validate_url(value, "url")
        self._url = value
    ...
    def set_field_at(self, index: int, *, name: str = None, value: str = None, inline: bool = False) -> "Embed":
        """
    ...
    def set_image(self, *, url: str, proxy_url: str = empty_field) -> "Embed":
        ...
        if proxy_url is empty_field:
            proxy_url = self._validate_url(url)
        else:
            proxy_url = self._validate_url(proxy_url, "image.proxy_url")

        if url is empty_field:
            url = self._validate_url(proxy_url)
        else:
            url = self._validate_url(url, "image.url")
    ...
    def set_thumbnail(self, *, url: str, proxy_url: str = empty_field) -> "Embed":
        ...
        if proxy_url is empty_field:
            ...
            proxy_url = self._validate_url(proxy_url, "thumbnail.proxy_url")

        if url is empty_field:
            url = self._validate_url(proxy_url)
        else:
            url = self._validate_url(url, "thumbnail.url")
    ...
    def set_footer(self, *, text: str = unspecified_field, icon_url: str = unspecified_field) -> "Embed":
        """
        Sets the footer; returns a reference to this object to allow for chaining.
        """
        if icon_url is not unspecified_field:
            icon_url = self._validate_url(icon_url, "footer.icon_url")
    ...
    def set_author(self, *, name: str, url: str = unspecified_field, icon_url: str = unspecified_field) -> "Embed":
        """
        Sets the author; returns a reference to this object to allow for chaining.
        """
        if url is not unspecified_field:
            url = self._validate_url(url, "author.url")

        if icon_url is not unspecified_field:
            icon_url = self._validate_url(icon_url, "author.icon_url")
    ...
    def _key_inclusion_predicate(self, name):
        return name[0] == "_" and hasattr(self, name) and getattr(self, name) is not unspecified_field
    ...
    def to_dict(self) -> dict:
        ...
        result = self._recursively_scrub_in_place(
            {key[1:]: getattr(self, key) for key in self.__slots__ if self._key_inclusion_predicate(key)}
        )
    ...
    @staticmethod
    def _validate_url(url, name="url") -> str:
        """
        ...
        Returns the valid URL (also prevents stupid asset errors everywhere due to dpy changes again)
        """
        url = str(url)
        try:
