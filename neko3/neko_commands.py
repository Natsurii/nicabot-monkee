...
class CommandMixin:
    ...
    @classmethod
    @neko3.functional.steal_signature_from(commands.command)
...
class GroupMixin:
    ...
    @neko3.functional.steal_signature_from(commands.Group.command)
    ...
    @neko3.functional.steal_signature_from(commands.Group.group)
...
class Group(GroupMixin, commands.Group, CommandMixin):
    """
    ...
    @neko3.functional.steal_signature_from(commands.Group.__init__)
    def __init__(self, func, *, invoke_without_command=True, **kwargs):
        ...
        commands.Group.__init__(self, func, invoke_without_command=invoke_without_command, **kwargs)
...
@neko3.functional.steal_signature_from(commands.command)
@neko3.functional.steal_docstring_from(commands.command)
def command(**kwargs):
    kwargs.setdefault("cls", Command)

    def decorator(coro):
        return commands.command(**kwargs)(coro)

    return decorator


#: Decorator for a group class. Works the same as the one in discord.ext.commands.
@neko3.functional.steal_signature_from(commands.group)
@neko3.functional.steal_docstring_from(commands.group)
def group(**kwargs):
    kwargs.setdefault("cls", Group)
...
async def get_usage(ctx):
    ...
    if isinstance(ctx.command, GroupMixin):
        for command in sorted(set(ctx.command.walk_commands()), key=str):
            try:
                suppressed = command.__command_attributes__.get("suppress_from_usage_message", False)
        ...
        lines.append(f"  {prefix}{ctx.command.signature}" if can_run else "  You lack permission to run this.")
    return lines
...
async def wait_for_edit(*, ctx, msg: typing.Optional[discord.Message] = None, timeout: float, custom_delete=None):
    """
    ...
    if not hasattr(ctx.command, "all_qualified_names"):
        raise TypeError(
            "This utility coroutine only works on command types " "that are derived from " "libneko.commands.Command"
        )
...
def acknowledge(ctx: Context, *, emoji: str = "\N{OK HAND SIGN}", timeout: typing.Optional[float] = 15) -> None:
    """
