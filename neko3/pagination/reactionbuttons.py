...
_logger = logging.getLogger("libneko.pagination.reactionbuttons")
...
class Button:
    ...
    def __init__(self, callback: CallbackT, *, emoji: str, name: Optional[str] = None) -> None:
        ...
        self.callback = neko3.functional.ensure_coroutine_function(callback)
    ...
    def invoke_if(self, predicate: InvokePredicateT) -> InvokePredicateT:
        ...
        """
        coro = neko3.functional.ensure_coroutine_function(predicate)
...
@button_class(emoji="\N{BLACK LEFT-POINTING DOUBLE TRIANGLE WITH VERTICAL BAR}\N{VARIATION SELECTOR-16}")
...
@button_class(emoji="\N{BLACK LEFT-POINTING TRIANGLE}\N{VARIATION SELECTOR-16}")
...
@button_class(emoji="\N{BLACK RIGHT-POINTING TRIANGLE}\N{VARIATION SELECTOR-16}")
...
@button_class(emoji="\N{BLACK RIGHT-POINTING DOUBLE TRIANGLE WITH VERTICAL BAR}\N{VARIATION SELECTOR-16}")
    ...
    async with lock:
        if btn.owner.owner == btn.owner.invoked_by.author:
            ...
            try:
                ...
                def check(message):
                    """Filters the message."""
                    return message.author == btn.owner.invoked_by.author and message.channel == btn.owner.channel
                ...
                if m_content == "*":
                    ...
                    try:
                        ...
                        if member.bot:
                            await btn.owner.send("Cannot transfer to a bot.", delete_after=10, add_to_list=False)
                            return
                        else:
                            btn.owner.owner = member
                    except commands.BadArgument:
                        return btn.owner.send(f"{m_content} was not a recognised member.", delete_after=10)
                ...
                _logger.exception(
                    "Unexpected error occurred. Poke the dev with a stick to fix this properly.", exc_info=True
                )
            else:
