...
_logger = logging.getLogger("libneko.pagination.navigator")
...
class BaseNavigator(abc.PagABC, Generic[PageT]):
    ...
    active_navigators = weakref.WeakSet()

    def __new__(cls, *args, **kwargs):
        new = super().__new__(cls)
        cls.active_navigators.add(new)
        return new
    ...
    def __init__(
        ...
        *,
        ...
        from neko3.pagination import Button
    ...
    def __await__(self):
        ...
        async def __awaitable__():
            await self.is_finished.wait()

        return __awaitable__().__await__()
    ...
    async def _on_reaction_add(self, reaction, user):
        ...
        if self.is_ready.is_set() and self._messages:
            ...
            is_in_whitelist = self.owner is None or user == self.owner or user == self.invoked_by.author
    ...
    def start(self):
        ...
        self._startup_task = self.loop.create_task(self._run())
        ...
        def on_done(_):
            try:
                ...
                _logger.exception(f"Exception occurred in {type(self).__name__}.start()", exc_info=True)
            finally:
                try:
                    self._startup_task = None
                finally:
    ...
    async def kill_and_await(self, action: CancelAction = CancelAction.REMOVE_ALL_MESSAGES):
        """Kills the paginator and waits for it to tidy up."""
        self.kill(action=action)
        await self
    ...
    async def _run(self):
        ...
        """
        self.pages = [*filter(None, self.pages)]
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    expected_reacts = [b.emoji for b in self.buttons.values() if await b.should_show()]
            ...
            if r & CancelAction.ADD_BOT_SHUTDOWN_REACT:
                await self.root_message.add_reaction("\N{MOBILE PHONE OFF}")
    ...
    async def _permission_error(self):
        try:
            ...
            _logger.exception("Another exception occurred when handling a permission error!", exc_info=True)
